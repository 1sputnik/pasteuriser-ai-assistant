...
void TInOCDFVanilaLSTM::count_gates(vector<OCDF>& samples) {
	...
	for (int i = 0; i < hid; i++) {
		...
		forgate_gate[i] = main_activator->activate(forgate_gate[i]); // sigm
		input_gate[i] = main_activator->activate(input_gate[i]); // sigm
		output_gate[i] = main_activator->activate(output_gate[i]); // sigm
		state_gate[i] = additional_activator->activate(state_gate[i]); // tanh
	}
}
...
void TInOCDFVanilaLSTM::count_short_memory(vector<OCDF>& samples) {
	for (int i = 0; i < hidden_range; i++) {
		...
		short_memory[(this->memory_bias + 1) * hidden_range + i] = output_gate[i] * additional_activator->activate(long_memory[(this->memory_bias + 1) * hidden_range + i]); // tanh
	}
}
...
vector<OCDF> TInOCDFVanilaLSTM::predict(vector<OCDF>& samples) {
	...
	if (short_memory.size() == 0) {
		long_memory = create_hollow_vector(this->hidden_range * 2);
		short_memory = create_hollow_vector(this->hidden_range * 2);
	}
...
void TInOCDFVanilaLSTM::save_model(std::string file_name) {
	...
	for (size_t i = 0; i < this->output_range; i++) {
		...
		for (size_t j = 0; j < this->input_range; j++) {
			save_file.write((char*)&U_y[j * output_range + i], sizeof(U_y[j * output_range + i]));
		}
...
void TInOCDFVanilaLSTM::load_model(std::string file_name) {
	...
	for (size_t i = 0; i < this->output_range; i++) {
		...
		for (size_t j = 0; j < this->input_range; j++) {
			load_file.read((char*)&U_y[j * output_range + i], sizeof(U_y[j * output_range + i]));
		}
